# Apresentação
```text
Graduado em engenharia de energia e em ciências e tecnologia (bacharel), pela Universidade Federal Rural do Semi-Árido (UFERSA). E especialista em análise e desenvolvimento de sistemas voltados a gestão empresarial, pelo Instituto Federal do Triangulo Mineiro (IFTM). Atualmente trabalha na Sankhya como desenvolvedor devops.

Como pesquisador, desenvolveu projetos de iniciação cientifica nas áreas de: sensoriamento remoto e processamento de imagens de satélites (LANDSAT-5, LANDSAT-7 e CBERS II); robótica educacional; e irrigação automatizada. Também atuou como professor de informática em escola particular; professor voluntário de física básica em curso pré-ENEM; e estagiário na Superintendência de Infraestrutura (SIN-UFERSA). No CEBRAC, onde ministrou os conteúdos de: informática básica; desenvolvimento de aplicações mobile em JAVA; e, de automação com Arduíno. Na faculdade UNA (Campus Karaíba), em Uberlândia-MG, atuou como técnico de laboratório de engenharias. Na CONPROVE Engenharia, atuou como técnico de eletrônica.
```
# Presentation
```text
Graduated in energy engineering and in science and technology (bachelor), from the Federal Rural University of the Semi-Árido (UFERSA). He is a specialist in analysis and development of systems aimed at business management, by the Federal Institute of Triangulo Mineiro (IFTM). He currently works at Sankhya as a devops developer.

As a researcher, he developed scientific initiation projects in the areas of: remote sensing and satellite image processing (LANDSAT-5, LANDSAT-7 and CBERS II); educational robotics; and automated irrigation. He also served as a computer teacher in a private school; volunteer teacher of basic physics in a pre-ENEM course; and intern at the Infrastructure Superintendence (SIN-UFERSA). At CEBRAC, where he taught the contents of: basic information technology; development of mobile applications in JAVA; and, automation with Arduino. At UNA College (Campus Karaíba), in Uberlândia-MG, he worked as an engineering laboratory technician. At CONPROVE Engenharia, he worked as an electronics technician.
```
